...
class StaticContext(): 
    ...
    def __init__(self, tokenizer, parentScript, parentBlock, 
                    inFunction, inForLoopInit, nesting, **rest): 
        self.tz = tokenizer
        self.funDecls = rest.get("funDecls") or []
        self.varDecls = rest.get("varDecls") or []
        self.ecmaStrictMode = rest.get("ecmaStrictMode") or False
        self.inFunction = inFunction or rest.get("inFunction") or False
        self.inForLoopInit = inForLoopInit or rest.get("inForLoopInit") or False
        self.nesting = nesting or rest.get("nesting") or NESTING_TOP
        self.parentScript = parentScript or rest.get("parentScript")
        self.parentBlock = parentBlock or rest.get("parentBlock")
        self.parenFreeMode = rest.get("parenFreeMode") or False
        self.ecma3OnlyMode = rest.get("ecma3OnlyMode") or False
        self.allLabels = rest.get("allLabels") or defs.Stack()
        self.currentLabels = rest.get("currentLabels") or defs.Stack()
        self.labeledTargets = rest.get("labeledTargets") or defs.Stack()
        self.defaultTarget = rest.get("defaultTarget")
        self.tokens = rest.get("tokens") or defs.tokens
        self.keywords = rest.get("keywords") or defs.keywords
    ...
    def Statements(self, n) :
        ...
        try :
            while ( not self.tz.done and self.tz.peek(True) != RIGHT_CURLY):
                stmt = self.Statement()
                # pass empty semicolon decorators
                if stmt.type_ == SEMICOLON and stmt.expression == None: pass
                else:
                    n.append(stmt)
        except Exception, e :
            if (self.tz.done):
    ...
    def Statement(self):
        ...
        #stmt_definition = tt in self.tokens and \
                #self.tokens.get(tt).title() + "Definition" or None
        ...
        # Skip identifier definition in statement form, it will defined as expression.
            ...
            #if self.nesting != NESTING_TOP:
                #type_ = STATEMENT_FORM
            #else:
                #type_ = DECLARED_FORM
            #n = getattr(self, stmt_definition)(type_)
          ...
          if n2 and n2.type_ == IN:
              n.type_ = FOR_IN
              n.iterator = n2[0]
              n.object = n2[1]

          elif (n2 and self.tz.match(IN)):
              n.type_ = FOR_IN
              ...
              tt2 = self.tz.peek()
              if self.parenFreeMode:
                isStatement = tt2 == LEFT_CURLY or defs.isStatementStartCode.get(tt2)
              else:
          ...
          raise self.tz.newSyntaxError(self.tokens[tt] + " without preceding try")
          ...
          return n
            ...
            if tt == IDENTIFIER:
                ...
                if tt == COLON:
                    label = t.token.value
                    if self.allLabels.has(label):
                        raise self.tz.newSyntaxError("Duplicate label")
                    t.get()
                    n = Node(self.tz, LABEL, label=label)
                    n.statement = self.Statement(self.pushLabel(label).nest(NESTING_SHALLOW))
                    if n.statement.type_ == LABEL:
                        n.target = n.statement.target 
                    else:
                        n.target = n.statement
                    return n
    ...
    def FunctionDefinition(self, requireName, functionForm) :
        ...
        x2 = self.static_context(self.tz, None, None, True, False, NESTING_TOP)
    ...
    def Variables(self, letBlock=None):
        ...
        while True:
            ...
            if (tt == LEFT_BRACKET or tt == LEFT_CURLY) :
                ...
                n2 = Identifier(self.tz, name = dexp, readOnly = n.type_ == CONST )
                n.append(n2)
            ...
            n2 = Identifier(self.tz, readOnly = n.type_ == CONST)
            n.append(n2)
    ...
    def ComprehensionTail(self) :
        ...
        while True:
            ...
            if tt in (LEFT_BRACKET, LEFT_CURLY): 
                ...
                n.iterator = n3 = Identifier(self.tz)
                n3.name = n3.value
    ...
    def HeadExpression(self) :
        ...
        if (p == END and not getattr(n, "parenthesized", None)) :
            tt = self.tz.peek()
            if (tt != LEFT_CURLY and not defs.isStatementStartCode.get(tt)):
    ...
    def PrimaryExpression(self):
        ...
        #expr_definition = tt in self.tokens and \
                #self.tokens.get(tt).title() + "Expression" or None

        #if expr_definition and callable(getattr(self, expr_definition, None)):
            #n = getattr(self, expr_definition)()
        ...
        if tt == FUNCTION:
                ...
                while True :
                    ...
                    if ((self.tz.token.value == "get" or self.tz.token.value == "set") and \
                        ...
                        if tt in (IDENTIFIER, NUMBER, STRING):
                            ...
                            if (self.tz.token.value in self.keywords) :
                                id = Node(self.tz, IDENTIFIER)
                                break
            ...
            if tt == NUMBER:
                ...
                n = Identifier(self.tz)
            elif tt == STRING:
                n = Node(self.tz, STRING, quotes=self.tz.token.quotes)
            else:
                n = Node(self.tz)

        else:
            #print "missing operand @", self.tz.token.lineno, self.tz.token.value
...
class Identifier(Node):
    def __init__(self, t, **kwargs):
        if "name" not in kwargs:
            kwargs["name"] = t.token.value
        Node.__init__(self, t, **kwargs)
