...
class Token :
    ...
    def getValue(self): getattr(self, "value_", None)
...
class Tokenizer() :
    ...
    def __init__(self, s, f="", l=1):
        ...
        self.opTokens = opTokens
        self.assignOps = defs.assignOps
        self.tokenIds = defs.tokenIds
        self.opTypeNames = defs.opTypeNames

    char = property(lambda self: self.source[self.cursor:self.cursor+1] or None)
    ...
    def mustMatches(self, l):
        tt = self.peek()
        match = False
        for t in l: 
            if t == tt: 
                match = True
        if not match: raise self.newSyntaxError("Missing " + l)
        self.get()
        return self.token
    ...
    def lexOp(self, ch) :
        ...
        op = node["op"]
        if (self.assignOps.has_key(op) and self.char == '=') :
            self.cursor += 1
            token.type_ = ASSIGN
            token.assignOp = self.tokenIds[dict(self.opTypeNames)[op]]
            op += '='
        else :
            token.type_ = self.tokenIds[dict(self.opTypeNames)[op]]
            token.assignOp = None
    ...
    def lexIdent(self, ch) :
        ...
        while True :
            ...
            if not ((ch >= 'a' and ch <= 'z') or (ch >= 'A' and ch <= 'Z') or
                    (ch >= '0' and ch <= '9') or (ch >= "\x7f" and ch <= "\xff") or 
                     ch == '$' or ch == '_'): 
                break
        ...
        token.type_ = self.keywords.get(id) or IDENTIFIER
        token.value = id
    ...
    def get(self, scanOperand=False) :
        ...
        if (ch >= 'a' and ch <= 'z') or \
                (ch >= 'A' and ch <= 'Z') or \
                (ch >= "\x7f" and ch <= "\xff") or \
                 ch == '$' or ch == '_':
            self.lexIdent(ch)
        elif (scanOperand and ch == '/') :
            self.lexRegExp(ch)
        elif (ch in self.opTokens) :
            self.lexOp(ch)
        elif (ch == '.') :
            self.lexDot(ch)
        elif (ch >= '1' and ch <= '9') :
